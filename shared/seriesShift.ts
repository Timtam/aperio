...
// does its UNTIL. That rewrite is a rule and lives in the core; the shell moves
// the start and the exceptions, which are instants in the device's zone. A rule
// that cannot move by whole days (the second Sunday of a month, a set position,
// a day past the 28th, …) comes back refused, and the surface offers to move
// only the occurrence instead.
...
/**
 * The rule for a series that starts on `startDayKey` (`YYYY-MM-DD`, device
 * zone) and moves by `days` whole days, with the time of day changing too when
 * `timeChanges`.
 */
