...
declare the lifecycle and the vtable; see [The Rust SDK](rust-sdk.md) for
...
See [the manifest reference](manifest.md) for every field.
...
[`hello-world` example](examples/hello-world.md) is exactly this, complete.

## Where to go next

- Don't want to hand-write each `ffi_*` wrapper? That's what the
  [Rust SDK](rust-sdk.md) macros are for.
- Building a *real* calendar adapter? Start from the
  [calendar-adapter-template](examples/calendar-adapter-template.md).
- Curious what crosses the boundary and why it stays compatible? Read
  [The C ABI](abi-reference.md).
