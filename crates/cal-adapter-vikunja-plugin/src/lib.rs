...
fn instance<'a>(
    handle: *mut c_void,
) -> Result<&'a PluginInstance<VikunjaAdapter>, PluginCallResult> {
    unsafe { PluginInstance::<VikunjaAdapter>::from_handle(handle) }
    ...
    Fut: std::future::Future<Output = CalResult<T>>,
{
    ...
    let p_static: &'static VikunjaAdapter =
        unsafe { std::mem::transmute::<&VikunjaAdapter, &'static VikunjaAdapter>(inst.plugin()) };
