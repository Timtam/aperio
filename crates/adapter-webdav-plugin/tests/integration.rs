...
fn make_manager() -> PluginManager {
    ...
    let desc: *mut AperioPlugin = unsafe { adapter_webdav_plugin::build_descriptor() };
    assert!(!desc.is_null());
    let destroy: unsafe extern "C" fn(*mut AperioPlugin) = adapter_webdav_plugin::DESTROY_FN;
    manager
