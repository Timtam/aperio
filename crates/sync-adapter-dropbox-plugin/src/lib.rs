...
async fn plugin_interactive_auth(args_json: String) -> Result<Vec<u8>, String> {
    ...
    match args.phase.as_deref() {
        Some("authorize") => {
            ...
            let authz = sync_adapter_dropbox::oauth::authorize(
                client_id,
                &redirect_uri,
                sync_adapter_dropbox::oauth::DROPBOX_AUTH_URL,
            )
        ...
        Some("exchange") => {
            ...
            let tokens = sync_adapter_dropbox::oauth::exchange_code(
                &http,
                sync_adapter_dropbox::oauth::DROPBOX_TOKEN_URL,
                client_id,
        ...
        None | Some("full") => {
            ...
            let http = reqwest::Client::new();
            let tokens =
                sync_adapter_dropbox::oauth::run(client_id, args.client_secret.trim(), &http)
                    .await
