//! Microsoft Teams videoconference adapter packaged as a
//! plugin (DESIGN.md §11 + §20).
//!
//! ## Init config
//!
//! ```json
...
//! ```
//!
//! Teams uses a PKCE-only public-client OAuth flow — there
//! is no `client_secret`. Rather than running its own sign-in
//! dance, this plugin re-uses the access token the host has
//! already minted for the Microsoft Graph calendar adapter:
//! the `access_token` field is sourced from the shared
//! keychain slot the `cal-adapter-microsoft-graph` adapter
//! uses, so the user only needs to authorise Microsoft once.
//! The current trait impl is a stub: all four FFI methods
//! route through to `TeamsAdapter`'s `Unsupported` returns
...
use vc_adapter_teams::{TeamsAccountConfig, TeamsAdapter};
use vc_core::{MeetingId, NewMeeting, VcAdapter};

plugin_sdk::vc_dispatch_helpers!(TeamsAdapter);
...
pub unsafe extern "C" fn plugin_open_instance(
    ...
    open_instance_with(config_json, |json| {
        ...
        {
            return Err(
                "client_id and access_token must not be empty".to_string(),
            );
        }
        Ok(TeamsAdapter::new(
            TeamsAccountConfig {
...
pub unsafe extern "C" fn plugin_close_instance(handle: *mut c_void) {
    PluginInstance::<TeamsAdapter>::drop_handle(handle);
}
...
plugin_sdk::declare_lifecycle! {
    id: "com.aperio.vc-adapter-teams",
    name: "Aperio Microsoft Teams",
    version: "0.1.0",
