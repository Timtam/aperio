//! Google Drive sync adapter packaged as a plugin (DESIGN.md §20).
//!
//! ## Init config
//!
//! ```json
...
//! ```
//!
//! Same OAuth-host-side model as the Dropbox plugin: the
//! refresh token comes from the host's keychain via
//! `config_json`. `folder_name` is the human-readable folder
