...
fn make_manager() -> PluginManager {
    ...
    let d: *mut AperioPlugin = unsafe { sync_adapter_googledrive_plugin::build_descriptor() };
    assert!(!d.is_null());
    let dx: unsafe extern "C" fn(*mut AperioPlugin) = sync_adapter_googledrive_plugin::DESTROY_FN;
...
fn open_one(manager: &PluginManager, refresh_token: &str) -> Arc<plugin_core::LoadedInstance> {
    let loaded = manager
        .get("com.aperio.sync-adapter-googledrive")
        .expect("registered");
