...
fn instance<'a>(
    handle: *mut c_void,
) -> Result<&'a PluginInstance<EwsAdapter>, PluginCallResult> {
    unsafe { PluginInstance::<EwsAdapter>::from_handle(handle) }
    ...
    Fut: std::future::Future<Output = CalResult<T>>,
{
    ...
    let p_static: &'static EwsAdapter =
        unsafe { std::mem::transmute::<&EwsAdapter, &'static EwsAdapter>(inst.plugin()) };
