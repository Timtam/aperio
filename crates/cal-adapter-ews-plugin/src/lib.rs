...
//! [`CalendarAdapterVtable`]: `IPF.Appointment` calendars,
//! `IPF.Task` task lists and `IPF.Contact` address books. One
//! [`EwsAdapter`] instance services every surface so the shared
//! HTTP client + listing caches stay coherent across reads.
//!
//! ## Init config
//!
//! ```json
...
//! `config_json` — same pattern as the OAuth-based sync plugins
//! from P4 (sync).
...
pub static PLUGIN_INSTANCE: PluginSingleton<EwsAdapter> =
    PluginSingleton::new();
...
pub unsafe extern "C" fn plugin_init(config_json: *const c_char) -> c_int {
    ...
    let cfg: InitConfig = match serde_json::from_str(json_str) {
        Ok(c) => c,
        Err(err) => {
            warn!(?err, "cal-adapter-ews-plugin: malformed init config");
            return plugin_sdk::plugin_core::PLUGIN_ERR_INVALID_CONFIG;
        }
    };
    ...
    let adapter = EwsAdapter::new(cfg.endpoint, creds);
...
where
    T: serde::Serialize,
    F: FnOnce(&'static EwsAdapter) -> Fut,
    Fut: std::future::Future<Output = CalResult<T>>,
{
    ...
    let p_static: &'static EwsAdapter =
        unsafe { std::mem::transmute::<&EwsAdapter, &'static EwsAdapter>(p) };
