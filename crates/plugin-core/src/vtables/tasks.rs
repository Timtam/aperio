...
unsafe impl crate::vtables::ForeignVtable for TasksVtable {
    // Revision 3 shipped 22 slots behind the leading `u32`, and the four
    // bytes that are now `struct_size` were padding it already had: 184
    // bytes. A literal, not `size_of::<Self>()` — see `read_vtable`. It stays
    // 184 when a slot is appended, because appending does not change a
    // struct that already shipped.
    const REVISION_3_SIZE: usize = 184;
}
...
const _: () = assert!(
    <TasksVtable as crate::vtables::ForeignVtable>::REVISION_3_SIZE
        <= std::mem::size_of::<TasksVtable>()
);
