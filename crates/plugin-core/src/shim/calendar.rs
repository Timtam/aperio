...
impl FfiCalendarAdapter {
    ...
    pub fn new(instance: Arc<LoadedInstance>) -> Option<Self> {
        ...
        let vtable = match unsafe { crate::vtables::read_vtable(outer.calendar) } {
            Ok(vtable) => vtable,
            ...
            Err(why) => {
                warn!(
                    plugin_id = %plugin.manifest.id,
                    %why,
                    "refusing to wrap the calendar surface: its vtable is unreadable",
                );
                return None;
            }
        };
        let vtable_ref: &CalendarVtable = &vtable;
