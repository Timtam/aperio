    ...
    pub fn new(instance: Arc<LoadedInstance>) -> Option<Self> {
        ...
        let Some(outer) = super::adapter_vtable(&plugin) else {
            warn!(
                ...
                "vc plugin has no vtable this host can read; refusing to wrap",
            );
            return None;
        };
        if outer.videoconference.is_null() {
        ...
        let vtable = match unsafe { crate::vtables::read_vtable(outer.videoconference) } {
            Ok(vtable) => vtable,
            ...
            Err(why) => {
                warn!(
                    plugin_id = %plugin.manifest.id,
                    %why,
                    "refusing to wrap the videoconference surface: its vtable is unreadable",
                );
                return None;
            }
        };
        let vtable_ref: &VcVtable = &vtable;
