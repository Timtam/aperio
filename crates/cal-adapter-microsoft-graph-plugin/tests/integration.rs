//! Smoke test for the Microsoft Graph plugin (P4 cal).
//!
...
fn shared_setup() {
    ...
    let rc = unsafe { cal_adapter_microsoft_graph_plugin::plugin_init(c.as_ptr()) };
...
fn register() -> PluginManager {
    ...
    let d: *mut AperioPlugin = unsafe {
        cal_adapter_microsoft_graph_plugin::aperio_plugin_create()
    };
    assert!(!d.is_null());
    let dx: unsafe extern "C" fn(*mut AperioPlugin) =
        cal_adapter_microsoft_graph_plugin::aperio_plugin_destroy;
