...
//! [`CalendarAdapterVtable`]: Outlook calendars + events,
//! Microsoft To Do tasks, Outlook contacts (incl. the Suggested
//! People sentinel). One [`MicrosoftGraphAdapter`] instance
//! services every surface so the shared OAuth token state +
...
//! ```
//!
//! Microsoft's PKCE OAuth dance happens host-side via
//! `MicrosoftGraphAdapter::authenticate_interactive` — the
//! resulting [`TokenSet`] is threaded into the plugin via
//! `config_json`. `authority` defaults to `"common"` so both
//! personal and work/school accounts work; admins may pin a
//! specific tenant.

use std::os::raw::{c_char, c_int};

use cal_adapter_microsoft_graph::{MicrosoftGraphAdapter, TokenSet, DEFAULT_AUTHORITY};
...
pub static PLUGIN_INSTANCE: PluginSingleton<MicrosoftGraphAdapter> =
    PluginSingleton::new();
...
pub unsafe extern "C" fn plugin_init(config_json: *const c_char) -> c_int {
    ...
    let cfg: InitConfig = match serde_json::from_str(json_str) {
        Ok(c) => c,
        Err(err) => {
            warn!(?err, "cal-adapter-microsoft-graph-plugin: malformed init config");
            return plugin_sdk::plugin_core::PLUGIN_ERR_INVALID_CONFIG;
        }
    };
    ...
    let adapter = MicrosoftGraphAdapter::new(cfg.client_id, cfg.authority, tokens);
...
where
    T: serde::Serialize,
    F: FnOnce(&'static MicrosoftGraphAdapter) -> Fut,
    Fut: std::future::Future<Output = CalResult<T>>,
{
    ...
    let p_static: &'static MicrosoftGraphAdapter = unsafe {
        std::mem::transmute::<&MicrosoftGraphAdapter, &'static MicrosoftGraphAdapter>(p)
    };
