...
pub fn run() {
    ...
    // so the cache re-bootstraps + re-maps with the new code. Idempotent (a
    // `user_prefs` generation marker); best-effort — logs and continues. When it
    // actually reset something, we kick an IMMEDIATE warm pass once the refresher
    // exists (see below) instead of waiting for the post-boot one — otherwise the
    // user sees stale events on launch and reaches for the manual re-sync (which
    // is exactly reset + trigger).
    let cache_generation_reset = {
        let shared = db.shared();
        let prefs = crate::user_prefs::UserPrefsRepo::new(&shared);
    ...
    let app = tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::list_day_markers,
            commands::create_day_marker,
            commands::update_day_marker,
            commands::delete_day_marker,
            commands::day_log,
            commands::day_logs_in_range,
            commands::set_day_log,
            ...
            commands::group_events,
            commands::ungroup_event,
            commands::dissolve_event_group,
            commands::event_groups_for_events,
            commands::heal_event_group_member,
            commands::refresh_event_group_signature,
            commands::decline_group_suggestion,
            commands::group_suggestion_declines,
            commands::search,
            ...
            commands::accept_remote_dataset_values,
            commands::adopt_local_dataset_values,
